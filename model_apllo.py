...
def reputation_computer(vehicle_info):
    ...
    vehicle_weight = vehicle_info[4]
    vehicle_direction = vehicle_info[5]
    reputation = (vehicle_type * 0.2 + vehicle_length * 0.1 + vehicle_width * 0.3 +
                  vehicle_weight * 0.1 + vehicle_direction * 0.3)
    reputation = torch.relu(reputation)
...
class Model(nn.Module):
    ...
    def reshape_for_lstm(self, feature):
        ...
        now_feat = feature.permute(0, 3, 2, 1).contiguous()  # to (N, V, T, C)permute方法将第二个和第三个维度交换。
    ...
    def forward(self, pra_x, pra_A, vehicle_rep, reputation, iteration, now_epoch, pra_pred_length,
        ...
        x_modified = x.clone()
        if x.shape[0] == 64:
            if iteration % 30 == 0 and x.shape[2] == 11 and iteration != 0:
                for t in range(x.shape[2]):
                    ...
                    for i in range(x.shape[0]):
                        ...
                        x_modified[i, :, t, :] = features[i, :, :]
                if now_epoch % 3 == 0:
                    print("————————star PPO————————")
                    x_modified = fusion_update(x_modified, reputation)
            else:
                x_modified = x
        ...
        now_predict_car = self.reshape_from_lstm(now_predict_car)
        ...
        now_predict_human = self.seq2seq_human(in_data=graph_conv_feature, last_location=last_position[:, -1:, :],
                                               ...
                                               teacher_location=pra_teacher_location)
        now_predict_human = self.reshape_from_lstm(now_predict_human)

        now_predict_bike = self.seq2seq_bike(in_data=graph_conv_feature, last_location=last_position[:, -1:, :],
                                             pred_length=pra_pred_length,
                                             teacher_forcing_ratio=pra_teacher_forcing_ratio,
                                             teacher_location=pra_teacher_location)
        now_predict_bike = self.reshape_from_lstm(now_predict_bike)
