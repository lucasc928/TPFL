...
class ApolloscapeDataset(BaseDataset):
    def __init__(self, obs_length, pred_length, time_step=0.2, sample_step=5):
        super().__init__(obs_length, pred_length, time_step)

        self.data_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../../data/CWL/NGSIM/")
        self.test_data_dir = os.path.join(self.data_dir, "prediction_test.new")
        self.val_data_dir = os.path.join(self.data_dir, "prediction_val.new")
        self.train_data_dir = os.path.join(self.data_dir, "prediction_train.new")
        self.data_path = {
    ...
    def format_data(self, data_dir, allow_incomplete_traces=True, allow_invisible_objects=True,
                    require_one_complete=True, require_one_visible=True):
